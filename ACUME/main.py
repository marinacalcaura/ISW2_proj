...
# Concatena il percorso corrente con il nuovo percorso
mypath = join(current_path, mypath)
print("Percorso completo:", mypath)

onlyfiles = [f for f in listdir(mypath) if isfile(join(mypath, f))]
print(" File trovati nella cartella:", onlyfiles)
...
for file_num, filename in enumerate(onlyfiles):
    ...
    for row_index, row in enumerate(file):
        ...
        true_count = sum(1 for data in data_rows if data.actual)
        false_count = sum(1 for data in data_rows if not data.actual)

        print(f" Numero di istanze 'True': {true_count}, Numero di istanze 'False': {false_count}")
    ...
    print(f" TP: {tpc}, FP: {fpc}, FN: {fnc}, TN: {tnc}")
    ...
    print("\nMETRICHE CALCOLATE PER:", filename)
    print(f" Total False Rows (total_nr_no): {total_nr_no}")
    print(f" Initial False Alarms (IFA_CNT): {IFA_CNT}")

    print("\n METRICHE DI CLASSIFICAZIONE:")
    print(f" Precision@0.5: {precision_0_5:.4f}")
    ...
    print(f" POP (Efficienza Predittiva): {pop:.4f}")
    print(f" Average POP*: {average_pop:.4f}")
    print(f" Average POFB: {average_pofb:.4f}")

    print("\n METRICHE BASATE SU POPT:")
    print(f" NPofBData (Popt Step Data): {average_npofb:.4f}")
    print(f" NPoptData (Popt Normalizzato): {average_npopt:.4f}")
